...
from django_drf_async.request import AsyncRequest
...
class AsyncAPIView(APIView):
    ...
    def _check_sync_permissions(
        self, request: AsyncRequest, permissions: Iterable[BasePermission]
    ):
    ...
    async def _check_async_permissions(
        self, request: AsyncRequest, permissions: Iterable[BasePermission]
    ):
    ...
    async def check_throttles(self, request: AsyncRequest) -> None:
        ...
        async_throttles = filter(
            lambda t: asyncio.iscoroutinefunction(t.allow_request), throttles
        )
        sync_throttles = filter(
            lambda t: not asyncio.iscoroutinefunction(t.allow_request), throttles
        )
        ...
        throttle_durations.extend(
            [
                duration
                async for duration in self._check_async_throttles(
                    request, async_throttles
                )
            ]
        )
